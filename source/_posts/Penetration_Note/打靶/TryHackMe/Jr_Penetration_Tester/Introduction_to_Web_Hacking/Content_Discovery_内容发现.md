---
title: Content_Discovery_内容发现
date: 2024-05-11 03:03:22
...
robots.txt文件是一个文档，它告诉搜索引擎他们是谁，哪些页面不允许显示在搜索引擎结果上，或者完全禁止特定的搜索引擎抓取网站。通常的做法是限制某些网站区域，以便它们不会显示在搜索引擎结果中。这些页面可能是管理门户或面向网站客户的文件等区域。该文件为我们提供了网站上所有者不希望我们作为渗透测试人员发现的位置的一个很好的列表。

## Favicon

网站图标是显示在浏览器地址栏或选项卡中的小图标，用于为网站打造品牌。

![](https://cdn.nlark.com/yuque/0/2024/png/39170111/1715397254672-9040c055-0f07-45a9-a018-a5e91cd50137.png)

**​**  

有时，当使用框架来构建网站时，作为安装一部分的网站图标会被遗留，如果网站开发人员不将其替换为自定义网站，这可以为我们提供有关正在使用的框架的线索。OWASP托管一个通用框架图标的数据库，您可以使用该数据库来检查目标网站图标

[OWASP favicon database - OWASP](https://wiki.owasp.org/index.php/OWASP_favicon_database)
```plain
...
```bash
curl https://static-labs.tryhackme.cloud/sites/favicon/images/favicon.ico | md5
```
```bash
...
当我们向 Web 服务器发出请求时，服务器会返回各种 HTTP 标头。这些标头有时可以包含有用的信息，例如 Web 服务器软件以及可能正在使用的编程/脚本语言。

```bash
curl http://10.10.100.153 -v
```

## Framework Stack

Once you've established the framework of a website, either from the above favicon example or by looking for clues in the page source such as comments, copyright notices or credits, you can then locate the framework's website. From there, we can learn more about the software and other information, possibly leading to more content we can discover.  
一旦你建立了一个网站的框架，无论是从上面的网站图标示例中，还是通过在页面源中寻找线索，如评论、版权声明或信用，你就可以找到框架的网站。从那里，我们可以了解有关软件和其他信息的更多信息，从而可以发现更多内容。

![image.png](https://cdn.nlark.com/yuque/0/2024/png/39170111/1715398003054-4eb4fab3-2744-435a-8814-d1edcef8fe46.png)
...
Google hacking / Dorking 利用 Google 的高级搜索引擎功能，可让您挑选自定义内容。

| **Filter** | **Example** | **Description** |
| --- | --- | --- |
| site | site:tryhackme.com | returns results only from the specified website address<br>仅返回来自指定网站地址的结果 |
| inurl | inurl:admin | returns results that have the specified word in the URL<br>返回 URL 中具有指定单词的结果 |
| filetype | filetype:pdf | returns results which are a particular file extension<br>返回作为特定文件扩展名的结果 |
| intitle | intitle:admin | returns results that contain the specified word in the title<br>返回标题中包含指定单词的结果 |
...
Wappalyzer ([Find out what websites are built with - Wappalyzer](https://www.wappalyzer.com/)) is an online tool and browser extension that helps identify what technologies a website uses, such as frameworks, Content Management Systems (CMS), payment processors and much more, and it can even find version numbers as well.  
Wappalyzer （https://www.wappalyzer.com/） 是一个在线工具和浏览器扩展，可帮助识别网站使用的技术，例如框架、内容管理系统 （CMS）、支付处理器等等，它甚至可以找到版本号。

## **Wayback Machine** **时光倒流机**

The Wayback Machine ([https://archive.org/web/](https://archive.org/web/)) is a historical archive of websites that dates back to the late 90s. You can search a domain name, and it will show you all the times the service scraped the web page and saved the contents. This service can help uncover old pages that may still be active on the current website.Wayback Machine （https://archive.org/web/） 是一个可追溯到 90 年代后期的网站历史档案。您可以搜索域名，它会向您显示该服务抓取网页并保存内容的所有时间。这项服务可以帮助发现当前网站上可能仍处于活动状态的旧页面。
...
S3 Buckets are a storage service provided by Amazon AWS, allowing people to save files and even static website content in the cloud accessible over HTTP and HTTPS. The owner of the files can set access permissions to either make files public, private and even writable. Sometimes these access permissions are incorrectly set and inadvertently allow access to files that shouldn't be available to the public. The format of the S3 buckets is http(s)://**{name}.**[s3.amazonaws.com](http://s3.amazonaws.com/) where {name} is decided by the owner, such as [http://tryhackme-assets.s3.amazonaws.com](http://tryhackme-assets.s3.amazonaws.com). S3 buckets can be discovered in many ways, such as finding the URLs in the website's page source, GitHub repositories, or even automating the process. One common automation method is by using the company name followed by common terms such as **{name}**\-assets, **{name}**\-www, **{name}**\-public, **{name}**\-private, etc.  
...
```plain
user@machine$ ffuf -w /usr/share/wordlists/SecLists/Discovery/Web-Content/common.txt -u http://10.10.100.153/FUZZ
```

**Using dirb:**

```plain
user@machine$ dirb http://10.10.100.153/ /usr/share/wordlists/SecLists/Discovery/Web-Content/common.txt
```

**Using Gobuster:**

```plain
user@machine$ gobuster dir --url http://10.10.100.153/ -w /usr/share/wordlists/SecLists/Discovery/Web-Content/common.txt
```
