---
title: AICryptoProxy：AI 赋能 Web 加密流量渗透测试完全指南
date: 2026-06-10 14:24:31
updated: 2026-06-10 14:24:31
tags:
  - AI
  - AI赋能
...
6. [默认模式 B：JSRPC Bridge——AI 零逆向桥接](#6-模式-bjsrpc-bridgeai-零逆向桥接)
7. [默认路由与备用切换](#7-默认路由与备用切换)
8. [常见问题与排错](#8-常见问题与排错)
9. [总结与展望](#9-总结与展望)
...
### 2.2 GitHub
> 对github项目进行二开：`https://github.com/hzhsec/AICryptoProxy`
>
...
为了直观理解 AI 做了什么，这里以默认的「模式 B：定位页面原生函数并注册 JSRPC action」为例：

| 步骤 | 传统人工操作 | AI 自动化操作 |
| --- | --- | --- |
| 1 | F12 打开 DevTools → Sources → 搜索 `encrypt` | `search_in_sources("encrypt")` → 自动扫描全部脚本 |
| 2 | 逐个文件翻阅搜索结果，判断哪一个是加密函数 | AI 自动理解代码语义，识别可调用入口 |
| 3 | 手动验证函数参数和返回值 | `hook_function("encryptFunc")` 自动追踪 |
| 4 | 分析 Key、IV、算法、混淆代码 | 默认不复现算法，直接调用页面原生函数 |
| 5 | 手动写桥接代码 | AI 生成 `jsrpc_inject.js` 并注册 `encrypt/decrypt` action |
| 6 | 手动写 mitmproxy 调用逻辑 | AI 生成 `jsrpc_client.py` 和双代理脚本 |
| 7 | 手动试错请求格式 | AI 先抓真实请求，再按 Content-Type 和字段结构回写 |
...
![](https://cdn.jsdmirror.com/gh/hzhsec/upload@main/20260506205030839.png)  
导入证书  
![](https://cdn.jsdmirror.com/gh/hzhsec/upload@main/20260506205151685.png)  
![](https://cdn.jsdmirror.com/gh/hzhsec/upload@main/20260506205243879.png)  
![](https://cdn.jsdmirror.com/gh/hzhsec/upload@main/20260506205305675.png)  
...
![](https://cdn.jsdmirror.com/gh/hzhsec/upload@main/20260506205507348.png)
...
│
├── target.com/                     # AI 按目标生成的输出目录
│   ├── downstream_jsrpc_proxy.py
│   ├── upstream_jsrpc_proxy.py
│   ├── jsrpc_client.py
│   ├── jsrpc_inject.js
│   └── start_proxy.py              # 一键启动双代理
|
|---skills  #jsrpc-mitm-auto 为默认 URL-only 入口，mitm-proxy 为模式A备用
│
...
```plain
[AI] 所有脚本已生成至 target.com/ 目录
[AI] 启动方式：
  1. Burp Suite → 监听 :8080
  2. python3 target.com/start_proxy.py
  3. 浏览器代理 → 127.0.0.1:8082
...
AI 同时生成 `downstream_jsrpc_proxy.py`、`upstream_jsrpc_proxy.py` 和 `start_proxy.py`。上下游脚本逻辑上与模式 A 相同，区别是加解密改为调用 `jsrpc_call()`；启动器负责复用/启动 JSRPC 并拉起双代理。
...
```plain
用户只提供 URL?
├── 是 → ★ 默认模式 B (JSRPC Bridge)
└── 否 → 用户是否明确要求 mitm-proxy / 模式 A / 提取 Key / 纯 Python?
        ├── 是 → ★ 模式 A (Direct Crypto)
        └── 否 → ★ 默认模式 B

模式 B 是否因 JSRPC/浏览器环境不可用而阻塞?
├── 是 → 评估切换模式 A 或提示最小人工动作
└── 否 → 保持模式 B，不再让用户选择
```

### 7.3 进阶策略：组合使用
场景：默认先用模式 B 快速搭建环境开始测试；只有长期稳定运行或浏览器依赖不可接受时，再让 AI 分析算法准备模式 A。
...
2. **默认只给 URL 即可**——系统优先走模式 B，模式 A 是显式备用方案
